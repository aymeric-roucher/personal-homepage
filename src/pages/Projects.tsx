...
const HuggingFaceIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 16 16" fill="currentColor" stroke="backgroundColor">
...
const Projects = () => {
  ...
  const renderProjectLinks = (links?: ContentListItem['links']) => {
    ...
    if (links.github_link) {
      linkItems.push(
        <a key="github" href={links.github_link} className="abbey-link flex items-center space-x-1" target="_blank" rel="noopener noreferrer">
    ...
    if (links.huggingface_link) {
      linkItems.push(
        <a key="huggingface" href={links.huggingface_link} className="abbey-link flex items-center space-x-1" target="_blank" rel="noopener noreferrer">
    ...
    if (links.webpage_link) {
      linkItems.push(
        <a key="webpage" href={links.webpage_link} className="abbey-link flex items-center space-x-1" target="_blank" rel="noopener noreferrer">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {loading ? (
            ...
            projects.map((project) => (
              <div key={project.slug} className="abbey-card group">
                ...
                <p className="text-muted-foreground text-sm mb-3">{project.metadata}</p>
