...
Since the critical band is wide at low frequencies and narrow (relative to pitch) at high ones, the same musical interval can be rough in the bass and smooth in the treble. Here is the Plomp-Levelt curve, plotted against the frequency ratio of the two tones. Drag the slider to move the pair up and down in register: the roughness bump marches toward unison as you go higher, because at a high pitch the same ratio is a much wider gap in Hertz, already outside the critical band and smooth, while down in the bass that identical ratio still packs both tones inside one band and grinds. Click any point on the curve to hear the two tones there:
...
Here is the full curve for two 6-harmonic tones, with the valleys marked; click anywhere to hear that interval:
...
- **The exact curve is soft.** Its precise shape varies across studies, ears, and loudness levels[^helmholtz], and summing the partial-pair roughnesses linearly is a convenient assumption, not a derived fact. The fine details move; the qualitative picture doesn't.
...
- **Volume is only half-modeled.** The $\min(l_1, l_2)$ rule does capture one real trick: a dissonant note played quietly adds spice without disrupting the harmony, which is how arrangers sneak ninths into lush voicings. But absolute level is absent: the same chord grinds more fortissimo than pianissimo, and the ear's sensitivity varies with frequency, none of which is in the model.
- **Harmony is dynamic, and this model is static.** It says nothing about dissonance deliberately sought to create emotion and motion. When the child in Schubert's *Erlkönig* cries "Mein Vater!" on an extremely dissonant minor ninth, the dissonance is the fear made audible, and its resolution, when the Father answers, is the comfort. On this post's map that cry is just a bad chord; in the song it is the whole point. Roughness alone cannot say why a composer climbs a peak on purpose, nor why a IV-V-I cadence pulls home.
