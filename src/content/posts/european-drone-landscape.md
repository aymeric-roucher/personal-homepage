...
The regulation that actually moves purchase orders is not a safety rule but a procurement one: [SAFE](https://ec.europa.eu/commission/presscorner/detail/en/ip_26_211) (Security Action for Europe), the EU's €150B loan program for defence purchases, only finances equipment whose component value is at least 65% European. A government that wants the cheap loans therefore has to buy drones built from European parts, which makes SAFE the one mechanism so far that steers buyers away from the Chinese supply chain part 4 is about; more on its geopolitics there.
...
All of that autonomy has to run on a computer that fits on the airframe and inside its power budget, which on a small drone means a few hundred grams and roughly 2 to 25 watts (see the chips in the table below), shared with everything else. Engineers call the constraint SWaP-C (size, weight, power, cost), and it bites hard: a stealthy micro-drone can't carry a noisy fan, so it cools passively through the airframe, and a chip that throttles under thermal load can slow its vision loop enough to crash the drone.
